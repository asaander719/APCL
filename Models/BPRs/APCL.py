...
class APCL(nn.Module):
    def __init__(self, args, embedding_weight, visual_features, text_features): 
        ...
        self.visual_nn = self._create_module(args.visual_feature_dim)
        self.p_visual_nn = self._create_module(args.visual_feature_dim)
        self.att_visual_nn = self._create_module(args.visual_feature_dim)

        if args.dataset == 'IQON3000':
            self.text_nn = self._create_module(100 * args.textcnn_layer)
            self.p_text_nn = self._create_module(100 * args.textcnn_layer)
            self.att_text_nn = self._create_module(100 * args.textcnn_layer)
        elif args.dataset == 'Polyvore':
            self.text_nn = self._create_module(args.text_feature_dim)
            self.p_text_nn = self._create_module(args.text_feature_dim)
            self.att_text_nn = self._create_module(args.text_feature_dim)
        ...
        if self.with_text:
            ...
            if args.dataset == 'IQON3000':
                embedding_weight = self._load_embedding_weight(args.textural_embedding_matrix, args.device)
    ...
    def _load_embedding_weight(self, textural_embedding_matrix, device):
        jap2vec = torch.load(textural_embedding_matrix)
        embedding_weight = []
        for jap, vec in jap2vec.items():
            embedding_weight.append(vec.tolist())
        embedding_weight.append(torch.zeros(300))
        embedding_weight = torch.tensor(embedding_weight, device=device, requires_grad=True)  # Set requires_grad to True
        return embedding_weight

    def _create_module(self, input_dim):
        module = nn.Sequential(
            nn.Linear(input_dim, self.hidden_dim),
            nn.Sigmoid()
        )
        module[0].apply(lambda m: uniform_(m.weight.data, 0, 0.001))
        module[0].apply(lambda m: uniform_(m.bias.data, 0, 0.001))
        return module
    ...
    def forward(self, batch, train, **args):
        ...
        if self.with_visual and self.with_text:
            if self.args.b_PC:
                ...
                # v_logits = torch.sum(J_visual_latent_p[:, None, :] * J_text_latent_p[:, :, None], dim=-1) / self.temperature
                # # 计算v和负样本的点积，然后除以温度
                # v_neg_logits = torch.sum(J_visual_latent_p[:, None, :] * K_text_latent_p[:, :, None], dim=-1) / self.temperature
                ...
                # t_logits = torch.sum(J_visual_latent[:, None, :] * J_text_latent[:, :, None], dim=-1) / self.temperature
                # # 计算v和负样本的点积，然后除以温度
                # t_neg_logits = torch.sum(J_visual_latent[:, None, :] * K_text_latent[:, :, None], dim=-1) / self.temperature
            ...
            if self.att:
                ...
                # v2_logits = torch.sum(J_visual_latent_p[:, None, :] * text_J_u[:, :, None], dim=-1) / self.temperature
                # # [:, None, :] 将 J 的形状从 [batch, output_dim] 转变为 [batch, 1, output_dim],torch.sum: [batch, output_dim]
                # v2_neg_logits = torch.sum(J_visual_latent_p[:, None, :] * text_K_u[:, :, None], dim=-1) / self.temperature
                ...
                # t2_logits = torch.sum(J_text_latent[:, None, :] * vis_J_u[:, :, None], dim=-1) / self.temperature
                # # 计算v和负样本的点积，然后除以温度
                # t2_neg_logits = torch.sum(J_text_latent[:, None, :] * vis_K_u[:, :, None], dim=-1) / self.temperature

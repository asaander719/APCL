    ...
    def forward(self, batch, train, **args):
        ...
        if self.with_visual:
            ...
            if self.att:
                ...
                all_u_pb_visual = self.p_visual_nn(v_sim_bs_out) #bs,512
                vis_J_u = self.p_visual_nn(vis_J)
                vis_K_u = self.p_visual_nn(vis_K)
        ...
        if self.with_text:
            ...
            if self.att:
                if self.args.dataset == 'IQON3000':
                    ...
                    all_u_pb_text = self.p_text_nn(t_sim_bs_out) #bs,512
                    text_J_u = self.p_text_nn(J_text_fea)
                    text_K_u = self.p_text_nn(K_text_fea)
                    ...
                    all_u_pb_text = self.p_text_nn(t_sim_bs_out) #bs,512
                    text_J_u = self.p_text_nn(text_J)
                    text_K_u = self.p_text_nn(text_K)
        ...
        # The paired RB experiment explicitly opts in to the indirect score.
        # Legacy scripts retain their previous behavior when the flag is absent.
        if getattr(self.args, 'include_indirect_score', False) and self.att and (self.with_visual or self.with_text):
            pred = pred + U_BuJ - U_BuK

...
def load_embedding_weight(textural_embedding_matrix, device):
    ...
    embedding_weight = [vec.tolist() for vec in jap2vec.values()]
    embedding_weight.append(torch.zeros(300, device=device))
    embedding_weight = torch.tensor(embedding_weight, device=device)
...
#     embeding_weight = torch.tensor(embeding_weight).to(args.device) 
#     return embeding_weight
def training(device, w_infoNCE, model, train_data_loader, optimizer, epoch):
    ...
    for iteration, aBatch in enumerate(train_data_loader):
        ...
        loss_meter.update(loss_scalar/iteration)
...
def main():
    ...
    visual_features_tensor = torch.cat((visual_features_tensor,v_zeros),0).to(args.device)
    ...
    if args.with_text:
        ...
        text_features_tensor = torch.cat((text_features_tensor,t_zeros),0).to(args.device)
        # text_features_tensor.to(args.device)
        if args.dataset == 'IQON3000':
            embedding_weight = load_embedding_weight(args.textural_embedding_matrix, args.device)#, args.device)#torch.Size([54276, 300])
        else:
